...
{
    public class StreamToStreamRunner : Runner
    {
        readonly Stream _src;
        public StreamToStreamRunner(ExifToolOptions opts, Stream src) : base(opts)
        {
        ...
        public override async Task<WriteResult> RunProcessAsync(IEnumerable<Operation> updates)
        {
            ...
            try
            {
                var cmd = Command.Run(_options.ExifToolPath, options: opts => { opts.StartInfo(si => si.Arguments = args); }) < _src > ms;
