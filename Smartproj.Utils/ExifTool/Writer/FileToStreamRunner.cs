...
{
    public class FileToStreamRunner : Runner
    {
        readonly string mSrc;
        public FileToStreamRunner(ExifToolOptions opts, string src) : base(opts)
        {
        ...
        public override async Task<WriteResult> RunProcessAsync(IEnumerable<Operation> updates)
        {
            ...
            var args = $"{updateArgs} -o - {EscapeFilePath(mSrc)}";
            var ms = new MemoryStream();

            try
            {
                var cmd = Command.Run(_options.ExifToolPath, options: opts => { opts.StartInfo(si => si.Arguments = args); }) > ms;
