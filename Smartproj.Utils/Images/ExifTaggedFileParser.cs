...
{
    public class ExifTaggedFileParser
    {
        public readonly ExifTaggedFileSegments Clusters;
        public int CriticalPixels { get; set; }
        public Logger ParserLog { get; set; }
        public TagFileTypeEnum Filter { get; set; }
        public ExifTaggedFileParser()
        {
            Clusters = new ExifTaggedFileSegments();
        }
        public List<ExifTaggedFile> Extract(string _path)
        {
            List<ExifTaggedFile> data = new List<ExifTaggedFile>();
            List<string> messages = new List<string>();
            List<string> warnings = new List<string>();
            List<string> errors = new List<string>();
            List<KeyValuePair<string, List<string>>> filesTree = new List<KeyValuePair<string, List<string>>>();

            FileProcess.ExtractFiles(_path, filesTree, Filter);
            ...
            for (int i = 0; i < filesTree.Count; i++)
            {
                var list = extractor.ExtractAllAsync(filesTree[i].Key);
                list.Wait();

                List<ExifTaggedFile> extractedList = new List<ExifTaggedFile>();

                for (int j = 0; j < filesTree[i].Value.Count; j++)
                {
                    IEnumerable<KeyValuePair<string, IEnumerable<JProperty>>> tags = null;
                    string key = Path.Combine(filesTree[i].Key, filesTree[i].Value[j]).Replace("\\", "/");
                    ...
                    if (list.Result.TryGetValue(key, out tags))
                    {
                        ...
                        TagFileTypeEnum fileNameType = filesTree[i].Value[j].ToFileType();
                        ...
                        {
                            ExifTaggedFile newitem1 = new ExifTaggedFile(index++, filesTree[i].Value[j], filesTree[i].Key, imageSize, fileCreateDate, fileNameType, tags, colorMode, hasGps, gps, colorProfile, bitsPerSampleValue, false, hasAlpha, false);
                ...
                {
                    fsSegment.ImportFiles(filesTree[i].Key, extractedList, x => Path.GetFileNameWithoutExtension(x.FileName), x => x.Index);
                }
            }
            ...
            if (data.Count > 0)
            {
                ...
                for (int i = 0; i < fsSegment.ChildNodes.Count; i++)
                {
                    ...
                    for (int j = 0; j < directory.ChildNodes.Count; j++)
                    {
                        ...
                        flatFilesTree.Add((ExifTaggedFileSegments)directory.ChildNodes[j]);
                    }
                }
            }
            ...
            if (ParserLog != null)
            {
                ParserLog.WriteAll("ExifTaggedFileParser.Extract", messages, warnings, errors);
            }
            messages.Clear();
            warnings.Clear();
            errors.Clear();
