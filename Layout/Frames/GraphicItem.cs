...
namespace Smartproj
{
    ...
    public class ImageFrame : ImageItem
    {
        ...
        /// 4: <see cref="AutoPositionObjectTypeEnum.ProtectFaces"/> - Масштабирование не происходит! Просходит попытка позиционировать изображение таким образом, чтобы области лиц не выходили за пределы зоны, ограниченной AutoFitMargins. При этом отступы AutoFitMargins в данном случае определены не только от обрезного формата, но и от позиции корешка (центра разворота), другими словами - от образного форматы полосы.
